												...
												while($row=$stmt->fetch(PDO::FETCH_ASSOC))
												{
												 ...
												 ?>
													<tr>
														...
														<td><?php echo $user ->getUsersFullName($row['Uploader']) ?></td>
														<td><?php if( $row['Imported']=="0"){echo "<span class='label label-sm label-warning'>Not Uploaded</span>";}else if( $row['Imported']=="1") { echo "<span class='label label-sm label-success'>Upoaded</span>";} else if( $row['Imported']=="2") { echo "<span class='label label-sm label-danger'>Suspended Account</span>"; } else  { echo "<span class='label label-sm label-warning'>Pending Admin Approval</span>"; }?></td>
														...
														<td>
															...
															<div class="hidden-sm hidden-xs btn-group">
																...
																$id = $row['File'];
																if("0"=="0"){
																	echo '<a class="btn btn-xs btn-success delete_user" data-id="'.$id.'" href="javascipt:void(0)">
																		<i class="ace-icon fa fa-upload bigger-120" title="import to database"></i>
																	</a>';
																}
																 ?>
															</div>
		...
		<script type="text/javascript">
				...
				$(document).ready(function(){
					...
					$('.delete_user').click(function(e){
						...
						bootbox.dialog({
							message: "Are you sure you want to import this file to the database ?",
							title: "<i class='ace-icon fa fa-upload'></i> Import !",
							buttons: {
							success: {
								label: "No",
								className: "btn-success",
							...
							danger: {
								...
								callback: function() {
									...
									$.post('../custom/upload.php', { 'upload':pid })
									.done(function(response){
										bootbox.alert(response);
										parent.fadeOut('slow');
										//location.reload();
